function [ftRank,ftScore] = ftSel_SVMRFECBR_ori(ft,label,l,t,threshold,savePath,isWM,WholeBrain)
% Feature selection using SVM-recursive feature elimination (SVM-RFE) with
%	correlation bias reduction (CBR).
...
%	B: Chemical, 2015
%
% Modified by Celine Provins on 05.2020


%% default parameters, can be changed by specifying the field in PARAM
% parameters for general SVM-RFE
nStopChunk = 1; % when number of features is less than this num, start 
...
Rth = threshold; % corrcoef threshold for highly corr features
...
while ~isempty(ftIn)
	...
	nFtIn = length(ftIn);
	[supVec,alpha_signed] = trainSVM(ft(:,ftIn),label,l,t,savePath,isWM,WholeBrain);
...
function [supVec,alpha_signed] = trainSVM(X,Y,l,t,savePath,isWM,WholeBrain)
% Run SVM and optimize parameter C 
model = fitcsvm(X, Y);%,'OptimizeHyperparameters',{'BoxConstraint'});
if isWM
    save(fullfile(savePath,strcat('SVModel_WM',int2str(t),int2str(l),'.mat')),'model');
else
    if WholeBrain
        save(fullfile(savePath,strcat('SVModel_YEO',int2str(t),int2str(l),'.mat')),'model');
    else
        save(fullfile(savePath,strcat('SVModel_AH',int2str(t),int2str(l),'.mat')),'model');
    end
end
supVec = full(model.SupportVectors);
alpha_signed = model.Alpha;
% svIdxs = model.sv_indices; % olderr versions of libSVM don't have it
